...
from src.rl_agents.ppo_family.continuous_ppomax import ContinuousPPOMaxAgent
from src.rl_agents.ppo_family.continuous_ppo import ContinuousPPOAgent
from src.rl_agents.ppo_family.continuous_ppoconditional import ContinuousPPOConditionalMaxAgent
...
rew_freqs_to_check = [4, 3]

# ~30 minutes/1e6 steps for PPO
# 4 frequencies, 2 algorithms: 4 hours per seed
...
for rew_freq in rew_freqs_to_check:
    ...
    for _ in range(N_SEEDS):
        ...
        # PPOMax
        run_name = 'PPOMax_freq=%s' % rew_freq
        agent = ContinuousPPOMaxAgent(make_env_fn, wandb_project_name=wandb_project_name, wandb_run_name=run_name,
                                      wandb_tags=wandb_tags, use_wandb=use_wandb, **ppo_max_config)
        agent.train(print_each=5)
